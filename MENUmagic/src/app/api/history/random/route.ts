...
export async function GET(request: NextRequest) {
  ...
  try {
    const menuRef = collection(db, "randomTable");
    const q = query(
      menuRef,
      where("userID", "==", userID),
...
function sortByCreatedAtDesc(data: any) {
  return data.sort((a: { timestamp: { seconds: number; nanoseconds: number; }; }, b: { timestamp: { seconds: number; nanoseconds: number; }; }) => {
    return (
      b.timestamp.seconds * 1000 + Math.floor(b.timestamp.nanoseconds / 1_000_000) -
      (a.timestamp.seconds * 1000 + Math.floor(a.timestamp.nanoseconds / 1_000_000))
    );
  });
}